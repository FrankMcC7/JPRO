...
def match_country_of_risk(
    ...
    if not correction_rows.empty:
        normalized = (
            correction_rows[label_column]
            .fillna("")
            .astype(str)
            .str.strip()
            .replace("", "(Blank)")
        )
        correction_rows = correction_rows.copy()
        correction_rows[label_column] = normalized
        grouped = (
            correction_rows.groupby(label_column)["Fund CoPER"]
            .apply(lambda series: join_ids_for_output(series.astype(str).tolist()))
            .reset_index(name="Fund CoPER IDs")
        )
        corrections_df = grouped
    else:
